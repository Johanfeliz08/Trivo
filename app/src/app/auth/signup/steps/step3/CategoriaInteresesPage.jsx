...
export default function CategoriaInteresesPage({ userData, setUserData }) {
  ...
  useEffect(() => {
    const response = async () => {
      try {
        const res = await api.get(`/category-interests/pagination?numeroPagina=${currentPage}&tamanoPagina=${itemsPerPage}`);

        if (res.status === 200) {
          const data = res.data;

          if (data.esExitoso && data.valor.elementos.length > 0) {
            let categoriasInteresesList = [];

            data.valor.elementos.map((categoriaInteres) => {
              categoriasInteresesList.push({
                categoriaId: categoriaInteres.categoriaInteresId,
                nombre: categoriaInteres.nombre,
              });
            });

            setCategoriasIntereses(categoriasInteresesList);
            setTotalPages(data.valor.totalPaginas);
            setCurrentPage(data.valor.paginaActual);
    ...
    // Llamar a la funcion para obtener las categorias de intereses
    response();
  }, [currentPage]);

  return (
    <>
      <div className="categoriasIntereses-container flex flex-row justify-center items-center gap-8">
        ...
        <div className="pagination-container flex flex-col justify-center items-center gap-14">
          <div className="categoriaInteres grid grid-cols-2 grid-rows-[repeat(4,_92px)] gap-4 justify-center items-start min-h-[368px]">
            {categoriasIntereses.map((categoriaInteres) => (
              <div className="categoriaInteres flex flex-row" key={categoriaInteres.categoriaId}>
                <input
                  className="hidden"
                  type="checkbox"
                  name={categoriaInteres.nombre}
                  id={categoriaInteres.categoriaId}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setUserData((prevData) => ({
                        ...prevData,
                        categoriasIntereses: [...prevData.categoriasIntereses, categoriaInteres.categoriaId],
                      }));
                    } else {
                      setUserData((prevData) => ({
                        ...prevData,
                        categoriasIntereses: prevData.categoriasIntereses.filter((i) => i !== categoriaInteres.categoriaId),
                      }));
                    }
                  }}
                />
                <label
                  htmlFor={categoriaInteres.categoriaId}
                  className={`flex relative justify-start items-center rounded-xl border-2 cursor-pointer shadow-sm bg-bg-secondary w-56 h-20 p-5 ${
                    userData.categoriasIntereses.find((i) => i === categoriaInteres.categoriaId) ? "border-primary" : "border-transparent"
                  }`}
                >
                  <div className={`icon absolute top-0 right-0 p-1 ${userData.categoriasIntereses.find((i) => i.categoriaId === categoriaInteres.categoriaId) ? "flex" : "hidden"}`}>
                  ...
                  <p className="font-medium text-lg">{categoriaInteres.nombre}</p>
                </label>
              </div>
            ))}
          </div>
          {errors.noData.error && (
            <div className="no-data flex justify-center items-center p-20">
